import networkx as nx, numpy as np, pandas as pd, cvxpy as cx
import Project1.A1.gudhi_persistence as gp
...
def simulate_contagion_map(params: dict):
    ...
    mapping_temp = {old: i for i, old in enumerate(DK.graph.nodes())}
    DK.graph = nx.relabel_nodes(DK.graph, mapping_temp, copy=False)
    seeding_method = params.get("seeding_method", "all_combinations")
    n_seeds = params.get("n_seeds", 2)

    seed_nodes = (0, 1)
    if seeding_method == "cluster_seeding":
        seed_nodes = initial_seed_nodes(graph=DK.graph, n_seeds=n_seeds,
                                        seed_cluster_distance=params.get("seed_cluster_distance", 10))
    elif seeding_method == "all_combinations":
        seed_nodes = get_seed_nodes_combinations(graph=DK.graph, n_seeds=n_seeds)

    return DK.graph, seed_nodes
...
def simulate_contagion_realization( graph: nx.Graph, init_seeds: tuple, params: dict,
    ...
    active_nodes_at_end, activation_times, snapshots = contagion_propagation(
        num_nodes=graph.number_of_nodes(), #params.get("num_nodes", 100),
